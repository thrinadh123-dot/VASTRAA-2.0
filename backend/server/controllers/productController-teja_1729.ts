...
export const getProducts = asyncHandler(async (req: Request, res: Response) => {
    ...
    const saleFilter = req.query.sale === 'true'
        ? {
            isOnSale: true
        }
        : {};

    const products = await Product.find({ ...keyword, ...saleFilter });
...
export const getProductById = asyncHandler(async (req: any, res: Response) => {
    try {
        ...
        ApiResponseHandler.error(res, error.message || 'Error retrieving product', 500);
    }
});
...
export const updateProduct = asyncHandler(async (req: any, res: Response) => {
    const { name, price, description, images, category, sizes, stock, image, countInStock, originalPrice, isOnSale } = req.body;
