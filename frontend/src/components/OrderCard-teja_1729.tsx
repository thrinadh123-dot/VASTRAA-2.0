            ...
            <div className="p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                    <h3 className="font-bold text-gray-900 text-lg mb-2 sm:mb-0">
                        Status: <span className={(order as any).deliveryStatus === 'Delivered' ? 'text-green-600' : 'text-blue-600'}>{(order as any).deliveryStatus || 'Processing'}</span>
                    </h3>
                </div>

                <div className="flex flex-wrap gap-4">
                    {((order as any).items || (order as any).orderItems).slice(0, 4).map((item: any, index: number) => (
                    ...
                    {((order as any).items || (order as any).orderItems).length > 4 && (
                        <div className="w-20 h-20 rounded-lg border border-gray-100 flex-shrink-0 bg-gray-50 flex items-center justify-center">
                            <span className="text-sm font-medium text-gray-500">+{(((order as any).items || (order as any).orderItems).length) - 4} more</span>
                        </div>
                    )}
                </div>
            </div>
