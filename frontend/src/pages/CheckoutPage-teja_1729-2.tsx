...
const OrderSummary = ({ items }: { items: any[] }) => {
    const itemsTotal = items.reduce((a, i) => {
        const price = (i.originalPrice && i.discountPercentage)
            ? Math.round(i.originalPrice * (1 - i.discountPercentage / 100))
            : i.price;
                    ...
                    <div key={`${item.product}-${i}`} className="flex items-center gap-3">
                        ...
                        <p className="text-xs font-bold text-[#1a1a1a] flex-shrink-0">
                            ₹{((item.originalPrice && item.discountPercentage
                                ? Math.round(item.originalPrice * (1 - item.discountPercentage / 100))
...
const CheckoutPage = () => {
    ...
    /* Totals */
    const itemsTotal = useMemo(() => cartItems.reduce((a, i) => {
        const price = (i.originalPrice && i.discountPercentage)
            ? Math.round(i.originalPrice * (1 - i.discountPercentage / 100))
            : i.price;
        return a + (price * i.quantity);
        ...
        try {
            ...
            const result = await dispatch(createOrder({
                ...
                orderItems: cartItems.map(item => ({
                    ...
                    name: item.name,
                    image: item.image,
                    price: item.originalPrice && item.discountPercentage
                        ? Math.round(item.originalPrice * (1 - item.discountPercentage / 100))
                        : item.price,
                    qty: item.quantity,
                    size: item.size
