                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        {item.originalPrice && item.originalPrice > item.price && (
                                                            <p className="text-[10px] text-[#aaa] line-through">₹{item.originalPrice.toLocaleString('en-IN')}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <p className="text-base font-bold text-[#1a1a1a] mb-2">{order.shippingAddress.fullName}</p>
                                    <div className="space-y-1">
                                        <p className="text-sm text-[#444] font-medium leading-relaxed">{order.shippingAddress.address}</p>
                                        <p className="text-sm text-[#444] font-bold">{order.shippingAddress.city}, {order.shippingAddress.state} – {order.shippingAddress.pincode}</p>
                                    </div>
                                    <div className="mt-6 pt-6 border-t border-[#f8f7f5] flex items-center gap-3">
                                        <FiMail className="text-[#ccc] w-4 h-4" />
                                        <p className="text-xs font-bold text-[#1a1a1a]">{order.shippingAddress.phone}</p>
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                ...
                                {['processing', 'confirmed'].includes(status) && (
                                    <button onClick={() => setIsCancelModalOpen(true)} className="flex items-center justify-between w-full px-5 py-4 bg-red-500/10 hover:bg-red-500/20 rounded-2xl transition-all group border border-red-500/10">
                                        <div className="flex items-center gap-3">
                                ...
                                <Link key={product._id} to={`/product/${product._id}`} className="group block">
                                    ...
                                    <div className="px-1 text-center">
                                        ...
                                        <div className="flex items-center justify-center gap-2">
                                            <p className="text-[13px] font-black text-[#1a1a1a] tracking-tight italic">
                                                ₹{(product.originalPrice && product.discountPercentage
                                                    ? Math.round(product.originalPrice * (1 - product.discountPercentage / 100))
                                                    : product.price).toLocaleString('en-IN')}
                                            </p>
                                            {product.originalPrice && (product.discountPercentage || product.originalPrice > product.price) && (
                                                <p className="text-[11px] text-[#aaa] line-through">₹{product.originalPrice.toLocaleString('en-IN')}</p>
                                            )}
                                        </div>
                                    </div>
                                </Link>
