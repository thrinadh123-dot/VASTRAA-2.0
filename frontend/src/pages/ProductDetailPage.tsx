...
const MOCK_REVIEWS = [
    { id: 1, name: 'Arjun Mehta', avatar: 'https://api.dicebear.com/7.x/personas/svg?seed=Arjun', rating: 5, date: '12 Jan 2025', text: 'Exceptional quality — fabric feels premium and the fit is exactly as described. Will definitely buy again.' },
    { id: 2, name: 'Priya Sharma', avatar: 'https://api.dicebear.com/7.x/personas/svg?seed=Priya', rating: 4, date: '28 Feb 2025', text: 'Love the minimalist design. Sizing is true to chart. Shipping was faster than expected.' },
    { id: 3, name: 'Rohan Das', avatar: 'https://api.dicebear.com/7.x/personas/svg?seed=Rohan', rating: 5, date: '03 Mar 2025', text: 'Best purchase this season. The material quality and finish are outstanding. Feels like luxury at an honest price.' },
];
...
const ProductDetailPage = () => {
    ...
    // A 24-hex-char string is a valid MongoDB ObjectId; anything else is a local slug
    const isMongoId = (val?: string) => /^[a-f\d]{24}$/i.test(val ?? '');
    ...
    // For local slugs, never show the backend loading spinner (no API call is made)
    const loading = backendLoading && isMongoId(id);
    ...
    // Local slugs (men-019, wmn-001 …) are served directly from local data — never hit the backend.
    ...
    useEffect(() => {
        ...
        // Skip the API call for local-only slugs — avoids the Cast to ObjectId error
    ...
    const toggleWishlist = async () => {
        ...
        if (!isMongoId(productId as string)) {
    ...
    const addToCartHandler = async () => {
        ...
        if (!userInfo || !isMongoId(productId as string)) {
    ...
    /* ── Rating breakdown (synthetic) ─────────────────────────────────────── */
    const rating = product?.rating ?? 0;
    const totalReviews = product?.numReviews || (product as any)?.reviews || 0;
    const breakdown = useMemo(() => {
        // Weighted fake breakdown centred on the rating
        const counts = [0, 0, 0, 0, 0];
        const total = Math.max(totalReviews, 50);
        counts[4] = Math.round(total * 0.55);  // 5★
        counts[3] = Math.round(total * 0.25);  // 4★
        counts[2] = Math.round(total * 0.12);  // 3★
        counts[1] = Math.round(total * 0.05);  // 2★
        counts[0] = total - counts[4] - counts[3] - counts[2] - counts[1]; // 1★
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-12">
                        ...
                        <div className="space-y-5">
                            <div className="flex items-end gap-2">
                                ...
                                <div className="pb-2">
                                    ...
                                    <p className="text-xs text-[#9a9a9a] mt-0.5">{totalReviews} New Reviews</p>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            {MOCK_REVIEWS.map(review => (
                                <div key={review.id} className="bg-[#fafaf9] rounded-2xl p-5 border border-[#f0f0f0]">
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <span className="text-[11px] text-[#9a9a9a]">{review.date}</span>
                                    </div>
