...
const OrderCard = ({ order }: { order: Order }) => {
    ...
    const handleCancel = async () => {
        if (window.confirm('Are you sure you want to cancel this order?')) {
            try {
                await dispatch(cancelOrder({ id: order._id, reason: 'Cancelled by user' })).unwrap();
...
const SizePreferencesTab = ({ userInfo }: { userInfo: any }) => {
    ...
    const getInitialPrefs = (gender: 'men' | 'women') => {
        const root = userInfo.sizePreferences || {};
        const genderPrefs = root[gender] || {};
                        ...
                        {currentCategories.map(cat => (
                            <div key={cat.key} className="bg-white rounded-2xl border border-[#f0eeec] shadow-sm p-6 overflow-hidden relative group">
                                <div className="absolute top-0 right-0 w-16 h-16 bg-[#f8f7f5] rounded-bl-full flex items-center justify-center pl-4 pb-4">
                                    <FiCheck className={`transition-all duration-300 ${prefs[cat.key] ? 'text-green-500 scale-100 opacity-100' : 'text-[#eee] scale-50 opacity-0'}`} />
                                </div>
...
/* ── Main Page ────────────────────────────────────────────────── */
type Tab = 'orders' | 'favourites' | 'personal' | 'sizes' | 'security' | 'advanced';
