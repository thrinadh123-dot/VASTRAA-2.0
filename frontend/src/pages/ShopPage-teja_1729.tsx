...
function sortProducts(list: Product[], sort: SortKey): Product[] {
    ...
    if (sort === 'reviews') return arr.sort((a, b) => (b.numReviews ?? 0) - (a.numReviews ?? 0));
    if (sort === 'discount') {
        return arr.sort((a, b) => {
            const getDisc = (p: any) => {
                if (p.discountPercentage) return Number(p.discountPercentage) / 100;
                if (p.originalPrice && p.price && p.originalPrice > p.price) {
                    return (Number(p.originalPrice) - Number(p.price)) / Number(p.originalPrice);
                }
                return 0;
            };
...
const ShopPage = () => {
    ...
    const filtered = useMemo(() => {
        ...
        // Enrich backend products with client-side isBestseller/isNewDrop flags
        // so style filters (bestseller, new) work even without seeded backend data
        const enriched = (products as any[]).map((p: any) => ({
            ...p,
            isBestseller: p.isBestseller || (p.rating >= 4.5 && (p.numReviews ?? 0) > 50),
            isNewDrop: p.isNewDrop || p.badge === 'NEW',
        }));
        let base = filterProducts(enriched as any, {
            category: urlCat,
    ...
    }, [products, urlCat, urlStyle, sort, priceFilter]);
                    ...
                    {productsLoading ? (
                        ...
                        >
                            {paginated.map(product => (
                                <VeloraProductCard key={product?._id || product.id} product={product} />
                            ))}
                        </motion.div>
                    )}
                </AnimatePresence>
