                            ...
                            {allOrders.map((order) => (
                                <tr key={order._id} className="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <p className="font-medium text-gray-900">{(order.user as any)?.username || 'Guest'}</p>
                                        <p className="text-[10px] text-gray-400 font-medium">{(order.user as any)?.email}</p>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${(order as any).deliveryStatus === 'Delivered' ? 'bg-blue-100 text-blue-600' :
                                            (order as any).deliveryStatus === 'Shipped' ? 'bg-yellow-100 text-yellow-600' :
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end space-x-2">
                                            ...
                                            {(order as any).deliveryStatus === 'Pending' && order.paymentStatus === 'Paid' && (
                                                <button
